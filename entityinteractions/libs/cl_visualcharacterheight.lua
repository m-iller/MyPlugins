...
do
    ...
    -- Main
    --
    local pTextureVCH = GetRenderTargetEx('_rt_VisualCharacterHeight', 1024, -- width
        1024, -- height
    ...
    function surface.GetVisualCharacterHeight(char, font)
        if not isstring(char) then assert(false, Format('bad argument #1 to \'GetVisualCharacterHeight\' (string expected, got %s)', type(char))) end
        --
        -- Manage the font
        --
        if font then
        ...
        if not CachedFont then
            ...
            if data then return data.Height, data.EmptySpace end
        end
